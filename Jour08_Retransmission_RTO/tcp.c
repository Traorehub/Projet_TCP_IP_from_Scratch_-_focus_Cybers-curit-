...
void tcp_handle_incoming(int fd, struct eth_hdr *eth, struct ip_hdr *ip) {
    ...
    if (conn) {
        uint32_t seg_seq = ntohl(tcp->seq); uint32_t seg_ack = ntohl(tcp->ack);
        ...
        if (conn->state == TCP_SYN_RECEIVED && (tcp->flags & TCP_ACK)) {
            if (seg_ack == conn->seq + 1) { 
                ...
                conn->last_send_time = time(NULL); conn->retransmit_count = 0;
            }
        }
        ...
        if (conn->state == TCP_SYN_RECEIVED || conn->state == TCP_ESTABLISHED) {
            if (difftime(now, conn->last_send_time) >= conn->curr_rto) {
                if (conn->retransmit_count >= MAX_RETRY) {
                    conn->state = TCP_CLOSED; continue;
                }
                printf("TCP TIMEOUT: Retransmission pour TCB %d (Essai %d/%d)\n", i, conn->retransmit_count + 1, MAX_RETRY);
                
                // Reconstruction simplifiée du paquet pour retransmission
                ...
                ip->ihl = 5; ip->version = 4; ip->len = htons(40); ip->proto = 6;
                ip->saddr = conn->saddr; ip->daddr = conn->daddr;
