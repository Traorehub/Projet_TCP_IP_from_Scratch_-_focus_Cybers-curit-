...
void tcp_handle_incoming(int fd, struct eth_hdr *eth, struct ip_hdr *ip) {
    ...
    if (tcp->flags & TCP_SYN) {
        ...
        tcp->flags = TCP_SYN | TCP_ACK; tcp->off = 5; tcp->win = htons(65535);
        uint32_t tmp_ip = ip->saddr; ip->saddr = ip->daddr; ip->daddr = tmp_ip;
        memcpy(eth->dmac, eth->smac, 6); memcpy(eth->smac, "\x00\x11\x22\x33\x44\x55", 6);
        ip->csum = 0; ip->csum = ip_checksum(ip, ip->ihl * 4);
    ...
    if (conn) {
        ...
        if (conn->state == TCP_SYN_RECEIVED && (tcp->flags & TCP_ACK)) {
            if (seg_ack == conn->seq + 1) { conn->state = TCP_ESTABLISHED; conn->seq = seg_ack; }
        }
